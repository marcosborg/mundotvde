        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.ownCarForm.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.own-car-forms.update", [$ownCarForm->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.ownCarForm.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $ownCarForm->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $ownCarForm->phone) }}" required>
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $ownCarForm->email) }}" required>
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('city') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="city" id="city" value="{{ old('city', $ownCarForm->city) }}">
                            @if($errors->has('city'))
                        ...
                        <div class="form-group {{ $errors->has('tvde') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="tvde" id="tvde" value="1" {{ $ownCarForm->tvde || old('tvde', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('tvde_card') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="tvde_card" id="tvde_card" value="{{ old('tvde_card', $ownCarForm->tvde_card) }}">
                        ...
                        <div class="form-group {{ $errors->has('message') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="message" id="message">{{ old('message', $ownCarForm->message) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('rgpd') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="rgpd" id="rgpd" value="1" {{ $ownCarForm->rgpd || old('rgpd', 0) === 1 ? 'checked' : '' }} required>
                        ...
                        <div class="form-group {{ $errors->has('obs') ? 'has-error' : '' }}">
                            <label for="obs">{{ trans('cruds.ownCarForm.fields.obs') }}</label>
                            <textarea class="form-control" name="obs" id="obs">{{ old('obs', $ownCarForm->obs) }}</textarea>
                            @if($errors->has('obs'))
                                <span class="help-block" role="alert">{{ $errors->first('obs') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.ownCarForm.fields.obs_helper') }}</span>
                        </div>
