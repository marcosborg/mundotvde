                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.receipts.update", [$receipt->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('reference') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="reference" id="reference" value="{{ old('reference', $receipt->reference) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('activity_launch') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="activity_launch_id" id="activity_launch_id" required>
                                @foreach($activity_launches as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('activity_launch_id') ? old('activity_launch_id') : $receipt->activity_launch->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
