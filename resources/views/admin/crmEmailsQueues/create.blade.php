        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.crmEmailsQueue.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.crm-emails-queues.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('stage_email') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="stage_email_id" id="stage_email_id" required>
                                @foreach($stage_emails as $id => $entry)
                                    <option value="{{ $id }}" {{ old('stage_email_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('to') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="to" id="to" value="{{ old('to', '') }}" required>
                            @if($errors->has('to'))
                        ...
                        <div class="form-group {{ $errors->has('body_html') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="body_html" id="body_html">{!! old('body_html') !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\CrmEmailsQueue::STATUS_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', 'queued') === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('error') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="error" id="error" value="{{ old('error', '') }}">
                            @if($errors->has('error'))
                        ...
                        <div class="form-group {{ $errors->has('scheduled_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="scheduled_at" id="scheduled_at" value="{{ old('scheduled_at') }}">
                        ...
                        <div class="form-group {{ $errors->has('sent_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="sent_at" id="sent_at" value="{{ old('sent_at') }}">
