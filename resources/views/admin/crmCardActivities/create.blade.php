        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.crmCardActivity.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.crm-card-activities.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('type') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\CrmCardActivity::TYPE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="type_{{ $key }}" name="type" value="{{ $key }}" {{ old('type', 'note') === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('meta_json') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="meta_json" id="meta_json">{{ old('meta_json') }}</textarea>
