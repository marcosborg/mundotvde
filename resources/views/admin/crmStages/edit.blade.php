        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.crmStage.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.crm-stages.update", [$crmStage->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="category_id" id="category_id" required>
                                @foreach($categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $crmStage->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $crmStage->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('position') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="position" id="position" value="{{ old('position', $crmStage->position) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('color') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="color" id="color" value="{{ old('color', $crmStage->color) }}">
                            @if($errors->has('color'))
                        ...
                        <div class="form-group {{ $errors->has('is_won') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="is_won" id="is_won" value="1" {{ $crmStage->is_won || old('is_won', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('is_lost') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="is_lost" id="is_lost" value="1" {{ $crmStage->is_lost || old('is_lost', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('auto_assign_to_user') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="auto_assign_to_user_id" id="auto_assign_to_user_id">
                                @foreach($auto_assign_to_users as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('auto_assign_to_user_id') ? old('auto_assign_to_user_id') : $crmStage->auto_assign_to_user->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
