        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.crmCard.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.crm-cards.update", [$crmCard->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="category_id" id="category_id" required>
                                @foreach($categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $crmCard->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('stage') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="stage_id" id="stage_id" required>
                                @foreach($stages as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('stage_id') ? old('stage_id') : $crmCard->stage->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('stage'))
                        ...
                        <div class="form-group {{ $errors->has('title') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="title" id="title" value="{{ old('title', $crmCard->title) }}" required>
                            @if($errors->has('title'))
                        ...
                        <div class="form-group {{ $errors->has('form') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="form_id" id="form_id">
                                @foreach($forms as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('form_id') ? old('form_id') : $crmCard->form->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('form'))
                        ...
                        <div class="form-group {{ $errors->has('source') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\CrmCard::SOURCE_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="source_{{ $key }}" name="source" value="{{ $key }}" {{ old('source', $crmCard->source) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('priority') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\CrmCard::PRIORITY_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="priority_{{ $key }}" name="priority" value="{{ $key }}" {{ old('priority', $crmCard->priority) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('status') ? 'has-error' : '' }}">
                            ...
                            @foreach(App\Models\CrmCard::STATUS_RADIO as $key => $label)
                                <div>
                                    <input type="radio" id="status_{{ $key }}" name="status" value="{{ $key }}" {{ old('status', $crmCard->status) === (string) $key ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('lost_reason') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="lost_reason" id="lost_reason" value="{{ old('lost_reason', $crmCard->lost_reason) }}">
                        ...
                        <div class="form-group {{ $errors->has('won_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="won_at" id="won_at" value="{{ old('won_at', $crmCard->won_at) }}">
                        ...
                        <div class="form-group {{ $errors->has('closed_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="closed_at" id="closed_at" value="{{ old('closed_at', $crmCard->closed_at) }}">
                        ...
                        <div class="form-group {{ $errors->has('due_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="due_at" id="due_at" value="{{ old('due_at', $crmCard->due_at) }}">
                        ...
                        <div class="form-group {{ $errors->has('assigned_to') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="assigned_to_id" id="assigned_to_id">
                                @foreach($assigned_tos as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('assigned_to_id') ? old('assigned_to_id') : $crmCard->assigned_to->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('created_by') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="created_by_id" id="created_by_id">
                                @foreach($created_bies as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('created_by_id') ? old('created_by_id') : $crmCard->created_by->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('position') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="position" id="position" value="{{ old('position', $crmCard->position) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('fields_snapshot_json') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="fields_snapshot_json" id="fields_snapshot_json">{{ old('fields_snapshot_json', $crmCard->fields_snapshot_json) }}</textarea>
