                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.recommendations.update", [$recommendation->id]) }}" enctype="multipart/form-data">
                        ...
                        <div class="form-group {{ $errors->has('driver') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="driver_id" id="driver_id" required>
                                @foreach($drivers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $recommendation->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('recommendation_status') ? 'has-error' : '' }}">
                            <label class="required" for="recommendation_status_id">{{ trans('cruds.recommendation.fields.recommendation_status') }}</label>
                            <select class="form-control select2" name="recommendation_status_id" id="recommendation_status_id" required>
                                @foreach($recommendation_statuses as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('recommendation_status_id') ? old('recommendation_status_id') : $recommendation->recommendation_status->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $recommendation->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $recommendation->email) }}" required>
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $recommendation->phone) }}" required>
                            @if($errors->has('phone'))
