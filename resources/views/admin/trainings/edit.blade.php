        ...
        <div class="panel-body">
          <form method="POST" action="{{ route("admin.trainings.update", [$training->id]) }}"
            ...
            <div class="form-group {{ $errors->has('image') ? 'has-error' : '' }}">
              <label for="image">{{ trans('cruds.training.fields.image') }}</label>
              <div class="needsclick dropzone" id="image-dropzone">
              </div>
              @if($errors->has('image'))
              <span class="help-block" role="alert">{{ $errors->first('image') }}</span>
              @endif
              <span class="help-block">{{ trans('cruds.training.fields.image_helper') }}</span>
            </div>
            <div class="form-group {{ $errors->has('text') ? 'has-error' : '' }}">
              <label for="text">{{ trans('cruds.training.fields.text') }}</label>
              <textarea class="form-control ckeditor" name="text"
                id="text">{!! old('text', $training->text) !!}</textarea>
              @if($errors->has('text'))
              <span class="help-block" role="alert">{{ $errors->first('text') }}</span>
              @endif
              <span class="help-block">{{ trans('cruds.training.fields.text_helper') }}</span>
            </div>
...
@if(isset($training) && $training->image)
      var file = {!! json_encode($training->image) !!}
