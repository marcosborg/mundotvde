        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.transferForm.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.transfer-forms.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('transfer_tour') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="transfer_tour_id" id="transfer_tour_id" required>
                                @foreach($transfer_tours as $id => $entry)
                                    <option value="{{ $id }}" {{ old('transfer_tour_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('obs') ? 'has-error' : '' }}">
                            <label for="message">{{ trans('cruds.transferForm.fields.obs') }}</label>
                            <textarea class="form-control" name="obs" id="obs">{{ old('obs') }}</textarea>
                            @if($errors->has('obs'))
                                <span class="help-block" role="alert">{{ $errors->first('obs') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.transferForm.fields.obs_helper') }}</span>
                        </div>
