        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.carRentalContactRequest.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.car-rental-contact-requests.store") }}" enctype="multipart/form-data">
                        @csrf
                        ...
                        <div class="form-group {{ $errors->has('obs') ? 'has-error' : '' }}">
                            <label for="obs">{{ trans('cruds.carRentalContactRequest.fields.obs') }}</label>
                            <textarea class="form-control" name="obs" id="obs" required>{{ old('obs') }}</textarea>
                            @if($errors->has('obs'))
                                <span class="help-block" role="alert">{{ $errors->first('obs') }}</span>
                            @endif
                            <span class="help-block">{{ trans('cruds.carRentalContactRequest.fields.obs_helper') }}</span>
                        </div>
