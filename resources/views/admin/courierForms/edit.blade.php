        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.courierForm.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.courier-forms.update", [$courierForm->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.courierForm.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $courierForm->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('phone') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="phone" id="phone" value="{{ old('phone', $courierForm->phone) }}" required>
                            @if($errors->has('phone'))
                        ...
                        <div class="form-group {{ $errors->has('email') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="email" name="email" id="email" value="{{ old('email', $courierForm->email) }}">
                            @if($errors->has('email'))
                        ...
                        <div class="form-group {{ $errors->has('city') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="city" id="city" value="{{ old('city', $courierForm->city) }}">
                            @if($errors->has('city'))
                        ...
                        <div class="form-group {{ $errors->has('courier') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="courier" id="courier" value="1" {{ $courierForm->courier || old('courier', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('account') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="account" id="account" value="{{ old('account', $courierForm->account) }}">
                        ...
                        <div class="form-group {{ $errors->has('rgpd') ? 'has-error' : '' }}">
                            <div>
                                <input type="checkbox" name="rgpd" id="rgpd" value="1" {{ $courierForm->rgpd || old('rgpd', 0) === 1 ? 'checked' : '' }} required>
