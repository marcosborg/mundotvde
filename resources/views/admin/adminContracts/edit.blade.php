        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.admin-contracts.update", [$adminContract->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('number') ? 'has-error' : '' }}">
                            <label class="required" for="number">{{ trans('cruds.adminContract.fields.number') }}</label>
                            <input class="form-control" type="number" name="number" id="number" value="{{ old('number', $adminContract->number) }}" step="1" required>
                        ...
                        <div class="form-group {{ $errors->has('driver') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="driver_id" id="driver_id" required>
                                @foreach($drivers as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('driver_id') ? old('driver_id') : $adminContract->driver->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('signed_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control date" type="text" name="signed_at" id="signed_at" value="{{ old('signed_at', $adminContract->signed_at) }}">
