        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.create') }} {{ trans('cruds.documentGenerated.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.document-generateds.store") }}" enctype="multipart/form-data">
                        @csrf
                        <div class="form-group {{ $errors->has('document_management') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="document_management_id" id="document_management_id" required>
                                @foreach($document_managements as $id => $entry)
                                    <option value="{{ $id }}" {{ old('document_management_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
