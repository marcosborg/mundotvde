        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                ...
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.vehicle-events.update", [$vehicleEvent->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('name') ? 'has-error' : '' }}">
                            <label class="required" for="name">{{ trans('cruds.vehicleEvent.fields.name') }}</label>
                            <input class="form-control" type="text" name="name" id="name" value="{{ old('name', $vehicleEvent->name) }}" required>
                            @if($errors->has('name'))
                        ...
                        <div class="form-group {{ $errors->has('description') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="description" id="description">{{ old('description', $vehicleEvent->description) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('vehicle_event_type') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_event_type_id" id="vehicle_event_type_id" required>
                                @foreach($vehicle_event_types as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vehicle_event_type_id') ? old('vehicle_event_type_id') : $vehicleEvent->vehicle_event_type->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('vehicle_event_warning_time') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="vehicle_event_warning_time_id" id="vehicle_event_warning_time_id">
                                @foreach($vehicle_event_warning_times as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('vehicle_event_warning_time_id') ? old('vehicle_event_warning_time_id') : $vehicleEvent->vehicle_event_warning_time->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('date') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="date" id="date" value="{{ old('date', $vehicleEvent->date) }}" required>
                            @if($errors->has('date'))
