        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.crmFormSubmission.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.crm-form-submissions.update", [$crmFormSubmission->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('form') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="form_id" id="form_id" required>
                                @foreach($forms as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('form_id') ? old('form_id') : $crmFormSubmission->form->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('form'))
                        ...
                        <div class="form-group {{ $errors->has('category') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="category_id" id="category_id" required>
                                @foreach($categories as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('category_id') ? old('category_id') : $crmFormSubmission->category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="form-group {{ $errors->has('submitted_at') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control datetime" type="text" name="submitted_at" id="submitted_at" value="{{ old('submitted_at', $crmFormSubmission->submitted_at) }}">
                        ...
                        <div class="form-group {{ $errors->has('user_agent') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="user_agent" id="user_agent" value="{{ old('user_agent', $crmFormSubmission->user_agent) }}">
                        ...
                        <div class="form-group {{ $errors->has('referer') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="referer" id="referer" value="{{ old('referer', $crmFormSubmission->referer) }}">
                        ...
                        <div class="form-group {{ $errors->has('utm_json') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="utm_json" id="utm_json">{{ old('utm_json', $crmFormSubmission->utm_json) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('data_json') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="data_json" id="data_json">{{ old('data_json', $crmFormSubmission->data_json) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('created_card') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="created_card_id" id="created_card_id">
                                @foreach($created_cards as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('created_card_id') ? old('created_card_id') : $crmFormSubmission->created_card->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
