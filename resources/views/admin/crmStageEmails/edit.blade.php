        ...
        <div class="col-lg-12">
            <div class="panel panel-default">
                <div class="panel-heading">
                    {{ trans('global.edit') }} {{ trans('cruds.crmStageEmail.title_singular') }}
                </div>
                <div class="panel-body">
                    <form method="POST" action="{{ route("admin.crm-stage-emails.update", [$crmStageEmail->id]) }}" enctype="multipart/form-data">
                        @method('PUT')
                        @csrf
                        <div class="form-group {{ $errors->has('stage') ? 'has-error' : '' }}">
                            ...
                            <select class="form-control select2" name="stage_id" id="stage_id" required>
                                @foreach($stages as $id => $entry)
                                    <option value="{{ $id }}" {{ (old('stage_id') ? old('stage_id') : $crmStageEmail->stage->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                                @endforeach
                            </select>
                            @if($errors->has('stage'))
                        ...
                        <div class="form-group {{ $errors->has('to_emails') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="to_emails" id="to_emails">{{ old('to_emails', $crmStageEmail->to_emails) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('bcc_emails') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control" name="bcc_emails" id="bcc_emails">{{ old('bcc_emails', $crmStageEmail->bcc_emails) }}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('subject') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="text" name="subject" id="subject" value="{{ old('subject', $crmStageEmail->subject) }}" required>
                        ...
                        <div class="form-group {{ $errors->has('body_template') ? 'has-error' : '' }}">
                            ...
                            <textarea class="form-control ckeditor" name="body_template" id="body_template">{!! old('body_template', $crmStageEmail->body_template) !!}</textarea>
                        ...
                        <div class="form-group {{ $errors->has('send_on_enter') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="send_on_enter" id="send_on_enter" value="1" {{ $crmStageEmail->send_on_enter || old('send_on_enter', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('send_on_exit') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="send_on_exit" id="send_on_exit" value="1" {{ $crmStageEmail->send_on_exit || old('send_on_exit', 0) === 1 ? 'checked' : '' }}>
                        ...
                        <div class="form-group {{ $errors->has('delay_minutes') ? 'has-error' : '' }}">
                            ...
                            <input class="form-control" type="number" name="delay_minutes" id="delay_minutes" value="{{ old('delay_minutes', $crmStageEmail->delay_minutes) }}" step="1">
                        ...
                        <div class="form-group {{ $errors->has('is_active') ? 'has-error' : '' }}">
                            <div>
                                ...
                                <input type="checkbox" name="is_active" id="is_active" value="1" {{ $crmStageEmail->is_active || old('is_active', 0) === 1 ? 'checked' : '' }}>
