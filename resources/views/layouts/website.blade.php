...
<head>
    ...
    <title>Mundo TVDE - @yield('title', 'Tudo o que precisa saber sobre transportes urbanos.')</title>
    ...
    <link rel="stylesheet" href="{{ asset('assets/website/css/template-premium.css?v=' . rand()) }}">

    @yield('styles')

    <script async src="https://www.googletagmanager.com/gtag/js?id=G-B5667Z67RW"></script>
    <script>
        window.dataLayer = window.dataLayer || [];
        function gtag() { dataLayer.push(arguments); }
        gtag('js', new Date());
...
<body>
    ...
    <main class="page landing-page">
        ...
        <section class="py-4 py-xl-5 section-soft">
            <div class="container">
                <div class="text-white border rounded d-flex flex-column justify-content-between align-items-center flex-lg-row p-4 p-lg-5"
                    style="background: linear-gradient(130deg, #312e81, #1d4ed8 40%, #0e7490);">
                    <div class="text-center text-lg-start py-3 py-lg-1">
                        <h2 class="fw-bold mb-2"><strong>Subscreva a nossa newsletter</strong></h2>
                        <p class="mb-0 text-white-50">Conheça as novidades do mundo TVDE</p>
                    </div>
                    <form class="d-flex justify-content-center flex-wrap my-2" method="post" id="newsletter" action="/forms/newsletter">
                        @csrf
                        <div class="my-2">
                            <input class="form-control" type="email" name="email" placeholder="O seu email">
                        </div>
                        <div class="my-2">
                            <button class="btn btn-light text-dark fw-semibold ms-sm-2" type="submit">Subscrever</button>
                        </div>
                    </form>
                </div>
            </div>
